...
export default function page() {
  ...
  const Task = ({ listItem }) => {
    ...
    return (
      <div
        ref={drag}
        className={`task flex flex-col justify-center items-start p-[14px] border rounded-lg border-[#DEDEDE] w-full bg-[#F9F9F9] ${
          ...
          <div
            className={`${
              listItem.priority === "Urgent"
                ...
                ? "bg-[#FFA235]"
                : "bg-[#0ECC5A]"
            } py-[6px] px-2 text-xs text-white rounded-lg mb-[13px]`}
          >
            {listItem.priority}
          </div>
        )}
        {listItem.deadline && (
          <div className="text-[14px] text-[#606060] flex flex-row gap-2 mb-4 font-semibold">
            <Image src={clock} alt="bar" width={24} height={24} />
            {/* {`${x.getFullYear()}-${x.getMonth() + 1}-${x.getDate()}`} */}
            {listItem.deadline}
          </div>
        )}
        <div className="text-[#797979] text-[14px]">
          {getTimeDifference(y, new Date())} ago
        </div>
      </div>
    );
  };
    ...
    const [{ isOver }, drop] = useDrop(() => ({
      ...
      drop: async ({ listItem }) => {
        ...
        if (result) {
          ...
          toast.error("Task status is not changed!");
        }
      },
      collect: (monitor) => ({
    ...
    return (
      <abbr title="Drag to delete task" ref={drop} className={`w-8 h-8 ${isOver && "bg-red-600 rounded"}`}>
        ...
        <MdDelete
          width={24}
          height={24}
          className="flex bg-red-500 rounded text-white p-1 w-full h-full"
        />
      </abbr>
    );
  };
  ...
  const Todo = ({ todo }) => {
    ...
    return (
      <div
        className={`lg:w-[24%] w-[320px] pb-5 flex flex-col justify-start p-3 items-start ${
          isOver && "bg-gray-100 rounded"
        } `}
      >
        <div
          ref={drop}
          className="flex flex-row w-full justify-between items-center pb-4"
        >
          <div className="text-center rounded text-[20px] font-semibold">
            Todo
          </div>
          <Image src={bars} alt="bar" width={24} height={24} />
        </div>
        <div
          className={`flex flex-col mb-4 w-full gap-4 ${
            isOver ? "bg-gray-100" : ""
          } `}
        >
          {todo?.map((listItem, key) => {
            return <Task key={key} listItem={listItem} />;
          })}
        </div>
        <div
          onClick={() => handleCreate("TODO")}
          className="w-full text-white p-2 rounded-lg bg-gradient-to-b from-[#3A3A3A] to-[#202020] flex flex-row justify-between items-center cursor-pointer"
        >
          Add new
          <GoPlus />
        </div>
  ...
  const UnderReview = ({ underreview }) => {
    ...
    return (
      ...
      >
        ...
        <div
          onClick={() => handleCreate("UNDER_REVIEW")}
          className="w-full text-white p-2 rounded-lg bg-gradient-to-b from-[#3A3A3A] to-[#202020] flex flex-row justify-between items-center cursor-pointer"
        >
          Add new
          <GoPlus />
  ...
  const Finished = ({ finished }) => {
    ...
    return (
      <div
        ref={drop}
        className={`xl:w-[24%] w-[320px] pb-5 flex flex-col justify-start p-3 items-start ${
          isOver ? "bg-gray-100 rounded" : ""
        } `}
      >
        <div className="flex flex-row w-full justify-between items-center pb-4">
  ...
  return (
    <main className="w-full max-h-screen overflow-hidden bg-whiteShade flex flex-row">
      <statusContext.Provider value={{ _status, set_status }}>
        {/* Side Layout  */}
        <div className="xl:w-[300px] w-[220px] bg-white border-r-[#DEDEDE] border-2 max-h-screen h-screen overflow-hidden text-whiteShade px-4 py-6 pt-8 hidden md:flex md:flex-col justify-between">
          <div>
            <div className="text-xl flex flex-row font-semibold pb-2">
              <Image
                ...
                className="pr-2"
              />
              <div className="text-[#080808] text-ellipsis overflow-hidden w-full">
                {user}
              </div>
            </div>
            <div className="pb-4">
              <div className="flex flex-row justify-between">
                ...
                <div
                  ...
                  className="text-[#797979] text-base px-2 py-3 rounded bg-[#F4F4F4] cursor-pointer"
                >
                  Log Out
                </div>
              </div>
            ...
            <button
              ...
              className="rounded-lg bg-gradient-to-b from-[#4B36CC] to-[#9C93D4] text-white p-2 text-center w-full flex flex-row items-center justify-center gap-2"
            >
              Create new task
        ...
        <div className="w-full relative overflow-y-auto overflow-x-hidden pl-4 pr-8 pt-6 flex min-h-screen  bg-[#F7F7F7] flex-col items-center justify-start ">
          <div
            className={`w-[200px] bg-dark max-h-screen h-screen overflow-hidden text-whiteShade p-4 md:flex-col md:hidden absolute top-0 left-[-150vw] shadow-lg shadow-gray-500 duration-300 ease-in-out ${
              toggleMenu && "translate-x-[150vw]"
            }`}
          >
            ...
            <div className="flex flex-col gap-1 w-full border-t-2 border-lightdark pt-4">
              <Link
                href={"/"}
                className="w-auto p-2 rounded hover:bg-lightdark border border-transparent cursor-pointer transform duration-[300ms] ease-in-out "
              >
                Home
              </Link>
              <Link
          ...
          <div
            className={`fixed top-0 right-[-80vw] duration-300 ease-in-out ${
              createToggle && "translate-x-[-80vw] z-30"
            }`}
          >
            <Create
              status={_status}
          ...
          <div className="w-full flex flex-col justify-between items-center ">
            ...
            <div className="flex flex-row gap-2 pb-4">
              <div className="rounded-lg py-6 px-4 flex flex-row gap-2 items-center justify-between bg-[#F4F4F4]">
                <Image
                  ...
                  width={76}
                  height={40}
                  className="w-[76px] h-[60px]"
                />
                <div>
                  <div className="text-base text-[#757575] font-semibold">
                    Introducing tags
                  </div>
            ...
            <div className="w-full flex flex-row justify-between pb-4 text-sm md::text-base">
              <div className="p-2 rounded-lg flex flex-row items-center bg-white max-w-32 lg:max-w-max ">
                <input
                  type="text"
                  placeholder="Search"
              ...
              <div className="flex flex-row gap-2 items-center">
                ...
                <button
                  onClick={() => setCreateToggle(!createToggle)}
                  className="rounded bg-gradient-to-b from-[#4B36CC] to-[#9C93D4] text-white text-center p-2  flex flex-row items-center justify-center gap-1"
                >
                  Create new task
                  <Image
                    src={create}
